-- rusticlientv2 - Fixed version with Spin Bot for everyone
...
local executorInfo = {pcall(identifyexecutor) and identifyexecutor() or 'Unknown'}
local executorName = type(executorInfo) == 'table' and executorInfo[1] or tostring(executorInfo)
...
for i, tabName in ipairs(tabs) do
    ...
    btn.Size = UDim2.new(1/5, -4, 1, -4)
    btn.Position = UDim2.new((i-1)/5, 2, 0, 2)
...
local Settings = {
    ...
    Spin = {
        SpinBot = false,
        SpinSpeed = 10,
        SpinDirection = 'Right' -- Right, Left, Random
    }
}
...
createSlider(combatScroller, 'Range', y, 'Combat', 'Range', 10, 30); y = y + 50
createToggle(combatScroller, 'Aim Assist', y, 'Combat', 'AimAssist'); y = y + 40
createSlider(combatScroller, 'FOV', y, 'Combat', 'FOV', 30, 180); y = y + 50
...
createToggle(renderScroller, 'ESP Health', y, 'Render', 'ESPHealth'); y = y + 40
createToggle(renderScroller, 'ESP Distance', y, 'Render', 'ESPDistance'); y = y + 40
createToggle(renderScroller, 'Wallhack', y, 'Render', 'Wallhack'); y = y + 40
...
local function doSpin()
    ...
    local direction = 1
    if Settings.Spin.SpinDirection == 'Left' then
        direction = -1
    elseif Settings.Spin.SpinDirection == 'Random' then
        direction = math.random() > 0.5 and 1 or -1
    end
    
    spinAngle = spinAngle + Settings.Spin.SpinSpeed * direction
...
print('📌 Телефон: кружок + кнопка | ПК: Right Shift')
print('🎯 Spin Bot работает для всех!')
